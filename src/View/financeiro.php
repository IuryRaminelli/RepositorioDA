...
$logExcluidos = $ConTransacao->selectAllLogTransacao();
...
$cor = $saldo < 0 ? 'red' : ($saldo > 0 ? 'green' : 'black');
...
<div class="container">
        ...
        <table class="table">
            ...
            <tbody>
                <?php
                foreach ($logExcluidos as $log) {
                    echo '<tr>
                        <td> R$ ' . $log['quantidade'] . '</td>
                        <td>' . (new DateTime($log['dia']))->format('d/m/Y') . '</td>
                        <td>' . $log['descricao'] . '</td>
                        <td>' . (new DateTime($log['data_exclusao']))->format('d/m/Y H:i:s') . '</td>
                    </tr>';
                }
                ?>
            </tbody>
