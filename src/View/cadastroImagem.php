    ...
    if(isset($_POST['cadastro'])){
      if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_FILES["arquivo"])){
          ...
          if (move_uploaded_file($_FILES["arquivo"]["tmp_name"], $target_file)){
              ...
              echo "<script>alert('Cadastrada com sucesso.'); window.location.href = '" . HOME . "CadastroImagem';</script>";
              header("Location: " . HOME . "CadastroAtividade");
          }else {
              echo "<script>alert('Desculpe, houve um erro ao enviar sua imagem.'); window.location.href = '" . HOME . "CadastroImagem';</script>";
          }
      }
  }
?>
